---
title: Sheep Dog
subtitle: Menez le troupeau, fermez le portail, visez les étoiles
pubDate: 2026-07-30
isPaid: true
platforms:
  - macOS
  - Windows
  - Linux
mainLinks:
  "Acheter sur Payhip (1,99 €)": "https://payhip.com/b/AI2h7"
links:
  "Boutique Noematic": "https://payhip.com/noematic"
  "Notes projet": "https://git.noematic.eu/baptiste/sheeps"
---

<div class="mb-6 rounded-2xl border border-lime-600/40 bg-lime-50 px-5 py-4 text-lime-950 dark:border-lime-400/30 dark:bg-lime-950/40 dark:text-lime-100">
  <p class="m-0 font-medium">Mini-jeu de berger cozy — 1,99 €</p>
  <p class="mt-2 mb-0 text-sm opacity-90">
    Un court jeu <strong>Godot</strong> desktop : guidez le chien, rassemblez les moutons, fermez le portail du parc, et battez les temps or / argent / bronze.
    Douze épreuves de campagne (dont tutoriels) plus des cartes aléatoires en mode endless.
  </p>
  <p class="mt-3 mb-0">
    <a href="https://payhip.com/b/AI2h7" class="inline-block text-center rounded-xl bg-lime-700 px-4 py-2 text-sm font-medium text-white hover:bg-lime-800">Acheter sur Payhip — 1,99 €</a>
  </p>
</div>

Sheep Dog est un petit score-attack pastoral : pas de simu de ferme, pas de multijoueur — juste vous, le troupeau et le chrono.

<br>

> Rentrez-les. Fermez le portail. Battez votre record.

### Obtenir le jeu

Builds desktop **macOS** et **Windows** (Godot 4). Téléchargement immédiat après achat.

<a href="https://payhip.com/b/AI2h7" class="btn text-white border border-primary-600/30 bg-primary-600/90 dark:bg-primary-800/80 hover:bg-primary-800 hover:border-primary-800 sm:mb-0 px-8 py-3 w-full rounded-3xl">Payhip — 1,99 €</a>

<p class="text-sm opacity-80">Prix « café » en achat unique. Aussi listé dans la <a href="https://payhip.com/noematic">boutique Noematic</a>.</p>

### Fonctionnalités

- **12 épreuves** — 2 tutoriels + 10 pâturages avec déblocages et étoiles  
- **Mode endless** — cartes procédurales pour rejouer  
- **Étoiles score-attack** — pars or / argent / bronze sur le HUD  
- **Style Pasture Soft** — silhouettes procédurales, palette douce  
- **Audio** — ambiance pré, bêlements, ding d’enclos, jingle de victoire ; mute avec **M**  
- **Clavier ou pointeur** — WASD / flèches ou souris / doigt  

### Contrôles

| Action | Entrée |
|--------|--------|
| Déplacer le chien | WASD / flèches ou souris / doigt |
| Ouvrir le portail | O (ou molette au portail) |
| Fermer le portail | F (ou molette au portail) |
| Pause | Échap |
| Muet | M |
| Rejouer (après victoire) | R |
| Épreuve suivante | Entrée |

**Victoire :** tous les moutons dans le parc **et** portail fermé.

### Conseils

#### Approchez par derrière

Les moutons fuient le chien. Arrivez derrière le troupeau pour le pousser vers le parc plutôt que de le disperser.

#### Discipline du portail

Ouvrez pour faire entrer les moutons, puis **fermez** pour valider. Un portail ouvert invite les fuites.

#### Des étoiles, pas de punition

Pas d’échec chronométré dur — prenez votre temps si besoin. Les runs rapides rapportent plus d’étoiles.

### Questions fréquentes {#faq}

#### Est-ce une simu de ferme ?

Non. C’est un court arcade de berger / cozy score-attack. Pas de cultures, pas de multi.

#### Seulement Mac et Windows ?

Les premiers builds visent **macOS** et **Windows**. Linux pourra suivre.

#### Y a-t-il une démo gratuite ?

Mini-jeu complet à 1,99 € sur [Payhip](https://payhip.com/b/AI2h7). Les retours sont bienvenus via le lien Support.

#### Jeux liés ?

[Air Fireman](/air-fireman) est notre arcade feux de forêt (classique gratuit dans le navigateur + remaster Godot). Sheep Dog est un titre cozy séparé.
